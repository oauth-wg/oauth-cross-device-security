...
In a cross-device flow, a user starts a scenario on the Consumption Device
...
An attacker attempts to access an adminstration portal repeatedly, generating a stream of authroization requests to the network administrator. The attempts are timed to occur while the administrator is asleep. The administrator is woken by the incoming requests on their phone, and in an attempt to stop the notifications they accidentally approve access and the attacker gains access to the portal.
